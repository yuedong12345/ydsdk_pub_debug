 ...
 * @param repeats if `YES`, `selector` will be invoked on `target` until the `MSAdWeakTimer` object is deallocated or until you call `invalidate`. If `NO`, it will only be invoked once.
...
/**
 * Creates an `MSAdWeakTimer` object and schedules it to start ticking inmediately.
 */
 ...
 * @note `MSAdWeakTimer` won't invoke the `selector` on `target` again after calling this method.
